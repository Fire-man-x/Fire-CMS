...
class FileUploadControl extends UploadControl
{
	...
	public static function register(Container $systemContainer, array $configuration = []): void
	{
		...
		Nette\Forms\Container::extensionMethod('addFileUpload', function (
			...
			if ($configuration['translator'] === null) {
				$translator = $systemContainer->getByType(Nette\Localization\Translator::class, false);
	...
	public function getUploadModel(): IUploadModel
	{
		...
		} elseif ($this->uploadModel instanceof IUploadModel) {
	...
	/**
	 * @param string|IUploadModel $uploadModel
	 */
	public function setUploadModel($uploadModel): self
	{
		if(!is_string($uploadModel) && !$uploadModel instanceof IUploadModel)
		{
			throw new Nette\InvalidArgumentException("Upload model '$uploadModel' must be instace of IUploadModel or string.");
		}
